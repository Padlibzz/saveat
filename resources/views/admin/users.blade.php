...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <div class="bg-white p-4 md:p-5 rounded-2xl shadow-xs border border-gray-100 flex flex-col justify-center">
            ...
            <h3 class="text-xl md:text-2xl font-black text-gray-800 mt-1">{{ \App\Models\User::count() }}</h3>
        </div>
        <div class="bg-white p-4 md:p-5 rounded-2xl shadow-xs border border-gray-100 flex flex-col justify-center">
            <p class="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-wider">Active Hari Ini</p>
            <h3 class="text-xl md:text-2xl font-black text-gray-800 mt-1">{{ \App\Models\User::where('status', 'aktif')->count() }}</h3>
        </div>
        <div class="bg-white p-4 md:p-5 rounded-2xl shadow-xs border border-gray-100 flex flex-col justify-center">
            <p class="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-wider">Baru Daftar</p>
            <h3 class="text-xl md:text-2xl font-black text-amber-600 mt-1">{{ \App\Models\User::whereDate('created_at', \Carbon\Carbon::today())->count() }}</h3>
        </div>
        <div class="bg-white p-4 md:p-5 rounded-2xl shadow-xs border border-gray-100 flex flex-col justify-center">
            <p class="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-wider">Dilaporkan</p>
            <h3 class="text-xl md:text-2xl font-black text-red-500 mt-1">{{ \App\Models\AbuseReport::count() }}</h3>
        </div>
    </div>
    ...
    <div class="space-y-3">
        @php
            $currentRole = request('peran', 'semua');
        @endphp
        ...
        <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-2 px-2 md:mx-0 md:px-0">
            <a href="{{ request()->fullUrlWithQuery(['peran' => null]) }}" 
               class="px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors shadow-sm
               {{ !$currentRole || $currentRole === 'semua' ? 'bg-[#545523] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50' }}">
                Semua
            </a>
            
            <a href="{{ request()->fullUrlWithQuery(['peran' => 'konsumen']) }}" 
               class="px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors shadow-sm
               {{ $currentRole === 'konsumen' ? 'bg-[#545523] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50' }}">
                Pelanggan
            </a>

            <a href="{{ request()->fullUrlWithQuery(['peran' => 'merchant']) }}" 
               class="px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors shadow-sm
               {{ $currentRole === 'merchant' ? 'bg-[#545523] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50' }}">
                Merchant
            </a>

            <a href="{{ request()->fullUrlWithQuery(['peran' => 'admin']) }}" 
               class="px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors shadow-sm
            ...
            @foreach($users as $user)
                <div class="bg-white p-4 rounded-2xl shadow-xs border border-gray-100 flex items-center justify-between hover:shadow-md transition-shadow">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="text-gray-400 hover:text-gray-700 p-2 transition-colors">
                        ...
                        <div x-show="open" @click.away="open = false" 
                            ...
                            <form action="{{ route('admin.ubahStatusUser', $user->id) }}" method="POST">
                                @csrf
                                ...
                                <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition">
                                    <i class="fa-solid fa-ban mr-2"></i>Blokir User
                                </button>
                            </form>
                            ...
                            <form action="{{ route('admin.ubahStatusUser', $user->id) }}" method="POST">
                                ...
                                <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-emerald-600 hover:bg-emerald-50 transition">
                                    <i class="fa-solid fa-check mr-2"></i>Aktifkan User
                                </button>
                            </form>
                        </div>
                    </div>
