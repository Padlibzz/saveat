...
@section('content')
    ...
    <div class="bg-[#FDFDF5] rounded-3xl border border-[#545523]/10 shadow-sm overflow-hidden max-w-4xl mx-auto">
        ...
        {{-- HEADER KARTU (Sesuai Persis di UI Foto) --}}
                            ...
                            <tr class="hover:bg-gray-50/40 transition-colors">
                                ...
                                <td class="py-4 px-6 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="/merchant/claim/{{ $item->id }}/detail" class="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded-xl font-bold hover:bg-gray-200 transition-all">
                                            Detail
                                        </a>
                                        <form action="/merchant/claim/{{ $item->id }}/verifikasi" method="POST">
                                            @csrf
                                            <button type="submit" class="text-xs bg-[#545523] text-[#F1F2CF] px-3 py-1.5 rounded-xl font-bold hover:bg-[#43441c] transition-all cursor-pointer">
                                                Selesai
                                            </button>
                                        </form>
                                    </div>
                ...
                @foreach($claims as $item)
                    <div class="p-5 space-y-3 bg-[#FDFDF5]">
                        ...
                        <div class="flex items-center gap-2 pt-1">
                            <a href="/merchant/claim/{{ $item->id }}/detail" class="flex-1 text-center text-xs bg-gray-100 text-gray-600 py-2.5 rounded-xl font-bold hover:bg-gray-200 transition-all">
                                Detail
                            </a>
                            <form action="/merchant/claim/{{ $item->id }}/verifikasi" method="POST" class="flex-1">
                                @csrf
                                <button type="submit" class="w-full text-center text-xs bg-[#545523] text-[#F1F2CF] py-2.5 rounded-xl font-bold hover:bg-[#43441c] transition-all cursor-pointer">
                                    Selesai Ambil
                                </button>
                            </form>
                        </div>
